...
class LIPParsingEdgeDataSet(data.Dataset):
    ...
    def resize_image_label(self, image, label, label_rev, size): 
        image = cv2.resize(image, size, interpolation = cv2.INTER_LINEAR) 
        label = cv2.resize(label, size, interpolation = cv2.INTER_NEAREST)
        label_rev = cv2.resize(label_rev, size, interpolation = cv2.INTER_NEAREST)
    ...
    def __getitem__(self, index):
        ...
        #first resize image to (473,473)
        image, label, label_rev = self.resize_image_label(image,label,label_rev,(473,473))
        size = image.shape 
        if self.is_mirror:
