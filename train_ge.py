...
def main():
    ...
    deeplab = Res_GE_multi(num_classes=args.num_classes) 
    ...
    criterion = CriterionCrossEntropyEdgeParsingAL()
    ...
    for i_iter, batch in enumerate(trainloader):
        ...
        if i_iter % args.save_pred_every == 0:
            ...
            pred_edges_colors = decode_predictions(preds[3], args.save_num_images, args.num_classes)
            for index, (img, lab) in enumerate(zip(images_inv, labels_colors)):
                writer.add_image('Images/'+str(index), img.transpose(2, 0, 1), i_iter)
                writer.add_image('Labels/'+str(index), lab.transpose(2, 0, 1), i_iter)
                writer.add_image('preds/'+str(index), preds_colors[index].transpose(2, 0, 1), i_iter)
                writer.add_image('edges/'+str(index), edges_colors[index].transpose(2, 0, 1), i_iter)
                writer.add_image('pred_edges/'+str(index), pred_edges_colors[index].transpose(2, 0, 1), i_iter)
